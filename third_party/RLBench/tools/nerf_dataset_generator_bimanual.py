...
def run_all_variations(task_name, headless, save_path, episodes_per_task, image_size):
    ...
    cam_mask = VisionSensor.create(resolution=camera_resolution, render_mode=RenderMode.OPENGL_COLOR_CODED) 
    ...
    cam_mask.set_parent(cam_placeholder) #_mask)

    cam_motion = CircleCameraMotion(cam, Dummy('cam_cinematic_base'), rotate_speed)
    cam_motion_mask = CircleCameraMotion(cam_mask, Dummy('cam_cinematic_base'), rotate_speed)
    # ========================================================================
    ...
    for task in tasks:
        ...
        for ex_idx in range(max_episode_number+1,episodes_per_task): 
            attempts = 50 # 10
            task_recorder = NeRFTaskRecorder(task_env, cam_motion, cam_motion_mask, fps=fps, num_views=num_views)
