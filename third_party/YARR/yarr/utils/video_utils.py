...
class NeRFTaskRecorder(object):
    ...
    def __init__(self, env: Environment, cam_motion: CameraMotion, cam_motion_mask: CameraMotion, fps=30, num_views=50):
        self._env = env
        self._cam_motion = cam_motion
        self._cam_motion_mask = cam_motion_mask
        self._fps = fps
    ...
    def take_snap(self, scene=None, obs: Observation=None): 
        ...
        all_intrinsics = []
        all_near_far = []
        all_mask = [] # new for Hierarchical Gaussian
        ...
        for i in range(self._num_views): # num views=50  50/ROUND we save 20-40 step 
            self._cam_motion.step()
            # self._cam_motion_mask.step()
            pose_matrix = self._cam_motion.cam.get_matrix()
            self._cam_motion_mask.cam.set_matrix(pose_matrix)   
            ...
            all_mask.append((self._cam_motion_mask.cam.capture_rgb() * 255.).astype(np.uint8))
    ...
    def save(self, path_dir):
        ...
        for t, all_views in enumerate(self._snaps_episode):
            ...
            for i, view in enumerate(all_views):
                ...
                # # for t, all_views in enumerate(self._mask_episode): 
                #     timestep_mask_dir = os.path.join(timestep_dir,'masks')
                #     os.makedirs(timestep_mask_dir, exist_ok=True)
                #     # for i, view in enumerate(all_views):           
                # save the mask
                img_path = os.path.join(timestep_mask_dir, str(i) + '.png')
                # view = Image.fromarray((view * 255).astype(np.uint8))
                mask_img = self._mask_episode[t][i]
                mask = cv2.cvtColor(mask_img, cv2.COLOR_RGB2BGR)
